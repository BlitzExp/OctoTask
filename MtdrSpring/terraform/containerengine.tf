resource "oci_containerengine_cluster" "mtdrworkshop_cluster" {
  ...
  kubernetes_version  = var.okeKubernetesVersion
  ...
  lifecycle {
    ...
    precondition {
      condition     = contains(data.oci_containerengine_cluster_option.mtdrworkshop_cluster_option.kubernetes_versions, var.okeKubernetesVersion)
      error_message = "La versión de Kubernetes ${var.okeKubernetesVersion} no es soportada por OCI en esta región. Versiones disponibles: ${join(", ", data.oci_containerengine_cluster_option.mtdrworkshop_cluster_option.kubernetes_versions)}"
    }
  }
}
